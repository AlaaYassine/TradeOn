...
$items = $form->posting("select ads.id as itemId, title, detail, country, City, type, Interestitem, image, ads.date, uId from ads left join favs on ads.id = favs.itemId WHERE type in ('Clothes') order by itemId desc");    ?>
<div id="wrapper">
...
$form = new items(); // function to post the items on the page located in the items.php page
$items = $form->loggedOut_posting("SELECT * FROM `ads` WHERE type in ('Clothes') order by id desc");
}
?>
</div>
</div></div>
